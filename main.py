...
def encrypt(txt,pwd):
    ...
    encList = [None] * 256
    #-----------------------------------------------------------------------------------------------------------------------------
    hashword = hashlib.sha512(pwd.encode()).hexdigest()  # Encode the password in sha512
    ...
    while len(encryption)<16:
        if (hashword[hashwLi[50]] not in encryption):
            ...
            hashword = h2h(hashword,iters)+h2h(hashword,iters+1)+h2h(hashword,iters+2)
    #-----------------------------------------------------------------------------------------------------------------------------
    txtEnc = enc(t2h(txt),encryption)  # Text in hexadecimal, then encrypted
    txtPos = hashwLi[:textMLen]

    txtLen = len(txtEnc)
    txtLenEnc = enc(format(txtLen, 'x').rjust(2,"0"),encryption)  # Text length in hexadecimal, then encrypted
